...
function getCellColorState(cell: IBufferCell): ColorState {
  return {
...
function colorStatesEqual(a: ColorState, b: ColorState): boolean {
  return (
...
function colorToCSS(colorMode: number, color: number): string {
  ...
  if (colorMode === 1) {
    if (color < 8) {
      ...
      const colors = ['#808080', '#ff0000', '#00ff00', '#ffff00', '#0000ff', '#ff00ff', '#00ffff', '#ffffff'];
...
function getSpanStyle(state: ColorState): React.CSSProperties {
  const style: React.CSSProperties = {};
  
  const fgColor = colorToCSS(state.fgColorMode, state.fgColor);
  const bgColor = colorToCSS(state.bgColorMode, state.bgColor);
  
  if (state.inverse) {
    if (fgColor) {style.backgroundColor = fgColor;}
    if (bgColor) {style.color = bgColor;}
    if (!fgColor && !bgColor) {
  ...
  if (state.blink) {style.animation = 'blink 1s infinite';}
  if (state.invisible) {style.visibility = 'hidden';}
  
  return style;
}

function lineToSpans(line: IBufferLine): React.ReactNode[] {
  ...
  const flushSpan = () => {
    if (currentText) {
      const style = currentState ? getSpanStyle(currentState) : {};
      spans.push(
  ...
  for (let i = 0; i <= lastNonSpaceIndex; i++) {
    ...
    const cellState = cell ? getCellColorState(cell) : null;

    if (!currentState || !cellState || !colorStatesEqual(currentState, cellState)) {
      flushSpan();
